...
Coding.generate = function(type) {
  ...
  var newNode;
  if (!node) {
    newNode = $tree.tree('appendNode', i)
    ...
    newNode = $tree.tree('addNodeAfter', i, node)
    $tree.tree('selectNode', newNode) // $tree.tree('selectNode', node.getNextNode())
  } else {
    newNode = $tree.tree('appendNode', i, node)
    $tree.tree('openNode', node)
    $tree.tree('selectNode', newNode)
  }
  Coding.clickNode(newNode)
}

Coding.clickNode = function(node) {
  var type = node.type
...
Coding.selectNode = function(node) {
  ...
  /*
    if (oldNode.type == 'trigger' || oldNode.type == 'alias') {
      var p = oldNode.patterns // Coding.lastState.patterns.split('\n')
  ...
  $('#QO-Coding-Dialog').dialog({
      ...
      buttons: {
        ["Delete permanently?"]: function() {
          ...
          // navigate to the surviving sibling
          if (n) { Coding.clickNode(n) }
          $(this).dialog('close')
        },
        Cancel: function() {
...
Coding.info = function(msg, type) {
  switch(type) {
    ...
    case 'success':
      ...
      break;
    default:
      msg = '<span class="self"><span class="black">&raquo; [<span class="sysecho">code</span>]</span> ' + msg + '</span><br />'
      break;
  }
  Core.printdown(msg)
}
...
Coding.split = function() {
  ...
  Coding.ace.renderer.updateFull();
  setTimeout(function() {
    Core._output.scrollTop(Core._output_.scrollHeight - Core._output_.scrollTop)
  }, 80)
}
